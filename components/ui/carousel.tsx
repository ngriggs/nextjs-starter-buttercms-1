	...
	return (
		<div ref={carouselRef} className="overflow-hidden">
			<div
				...
				className={cn(
					"flex",
					orientation === "horizontal" ? "mx-0 " : "-mt-4 flex-col",
					className
				)}
				{...props}
			/>
...
const CustomCarouselNavigation = ({
	...
	React.useEffect(() => {
		const updateActiveSlides = () => {
			...
			setActiveSlide(selectedSlide || 0);
		};

		// Initialize active slides on mount
		updateActiveSlides();

		// Listen for changes in slide position
		api?.on("select", updateActiveSlides);

		return () => {
			// Cleanup event listener
			api?.off("select", updateActiveSlides);
		};
	}, [api]);

	const isVertical = orientation === "vertical";
	const isRightPlacement = buttonPlacement === "right";

	return (
		<div
			className={cn(
				`mx-auto my-auto flex ${
					isVertical
...
export { CustomCarouselNavigation as CarouselNavigation };
